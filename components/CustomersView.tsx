...
const CustomersView: React.FC<CustomersViewProps> = ({ customers, sales, onAddCustomer, onUpdateCustomer, onDeleteCustomer }) => {
    ...
    const handleSelectAll = (e: React.ChangeEvent<HTMLInputElement>) => {
        if (e.target.checked) {
            setSelectedInvoiceIds(new Set(customerSales.map(s => s.id)));
        } else {
            setSelectedInvoiceIds(new Set());
        }
    };

    const handleGenerateSelectedStatement = () => {
        if (!selectedCustomer || selectedInvoiceIds.size === 0) return;

        const selectedSales = customerSales.filter(s => selectedInvoiceIds.has(s.id));
        ...
        const totalDemanding = totalInvoiced - totalPaid;
        ...
        const html = `
            ...
            <head>
                ...
                <style>
                    ...
                    .brand { font-size: 24px; font-weight: 900; text-transform: uppercase; color: #1a2232; }
                    .client-info { margin-bottom: 30px; }
                    .client-info h2 { font-size: 18px; margin: 0 0 5px 0; }
                    .client-info p { margin: 0; font-size: 12px; color: #718096; }
                    .summary-grid { display: grid; grid-template-cols: repeat(3, 1fr); gap: 20px; margin-bottom: 40px; }
                    .stat-card { padding: 20px; border-radius: 12px; border: 1px solid #edf2f7; text-align: center; }
                    .stat-card h4 { margin: 0 0 5px 0; font-size: 10px; text-transform: uppercase; color: #a0aec0; letter-spacing: 1px; }
                    .stat-card p { margin: 0; font-size: 20px; font-weight: 800; }
                    table { width: 100%; border-collapse: collapse; font-size: 11px; }
                    th { text-align: left; background: #f8fafc; padding: 12px; text-transform: uppercase; color: #4a5568; border-bottom: 2px solid #edf2f7; }
                    td { padding: 12px; border-bottom: 1px solid #edf2f7; vertical-align: top; }
                    .payment-row { font-size: 10px; color: #10b981; margin-top: 4px; font-weight: 600; }
                    .item-row { font-size: 10px; color: #4a5568; margin-top: 2px; display: flex; justify-content: space-between; border-bottom: 1px dashed #eee; padding-bottom: 2px; }
                    .text-right { text-align: right; }
                    .footer { margin-top: 50px; text-align: center; font-size: 10px; color: #a0aec0; border-top: 1px solid #edf2f7; padding-top: 20px; }
                </style>
            </head>
            <body>
                <div class="header">
                    <div class="brand">Eko Prints</div>
                    <div style="text-align: right">
                        <div style="font-weight: 800; font-size: 14px;">CUSTOMER STATEMENT</div>
                        <div style="font-size: 10px; color: #718096;">Generated: ${new Date().toLocaleString()}</div>
                    </div>
                </div>
                ...
                <div class="summary-grid">
                    <div class="stat-card"><h4>Total Invoiced</h4><p>${formatUGX(totalInvoiced)}</p></div>
                    <div class="stat-card"><h4>Total Paid</h4><p style="color: #10b981">${formatUGX(totalPaid)}</p></div>
                    <div class="stat-card"><h4>Total Demanding</h4><p style="color: #ef4444">${formatUGX(totalDemanding)}</p></div>
                </div>
                <table>
                    <thead>
                        <tr>
                            <th>Invoice Ref / Details</th>
                            <th>Date</th>
                    ...
                    <tbody>
                        ${selectedSales.map(sale => `
                            <tr>
                                <td>
                                    <div style="font-weight: 800; color: #1a2232; margin-bottom: 8px;">#${sale.id.substring(0, 8).toUpperCase()}</div>
                                    
                                    <div style="margin-bottom: 10px;">
                                        <div style="font-size: 9px; font-weight: 800; color: #a0aec0; text-transform: uppercase; margin-bottom: 4px;">Items Breakdown:</div>
                                        ${sale.items.map(item => `
                                            <div class="item-row">
                                                <span style="flex: 1;">${item.name}</span>
                                                <span style="width: 40px; text-align: center;">x${item.quantity}</span>
                                                <span style="width: 80px; text-align: right;">@ ${formatUGX(item.price).replace(' UGX', '')}</span>
                                                <span style="width: 80px; text-align: right; font-weight: 700;">${formatUGX(item.price * item.quantity).replace(' UGX', '')}</span>
                                            </div>
                                        `).join('')}
                                    </div>

                                    ${(sale.payments || []).length > 0 ? `<div style="font-size: 9px; font-weight: 800; color: #a0aec0; text-transform: uppercase; margin-bottom: 4px;">Payment History:</div>` : ''}
                                    ${(sale.payments || []).map(p => `<div class="payment-row">↳ Paid: ${new Date(p.date).toLocaleDateString()} - ${formatUGX(p.amount)}</div>`).join('')}
                                </td>
                                <td>${new Date(sale.date).toLocaleDateString()}</td>
                                <td class="text-right">${formatUGX(sale.total)}</td>
                                <td class="text-right">${formatUGX(sale.amountPaid || 0)}</td>
                                <td class="text-right" style="font-weight: 800; color: ${sale.total - (sale.amountPaid || 0) > 0 ? '#ef4444' : '#10b981'}">${formatUGX(sale.total - (sale.amountPaid || 0))}</td>
                            </tr>
                        `).join('')}
                    </tbody>
                </table>
                <div class="footer">This is a computer-generated statement. Eko Prints Management System.</div>
            </body>
            </html>
        `;
                        ...
                        <div>
                            <div className="flex justify-between items-center mb-4 ml-1">
                                ...
                                {selectedInvoiceIds.size > 0 && (
                                    <button
                                        onClick={handleGenerateSelectedStatement}
                                        className="flex items-center bg-blue-600 text-white px-4 py-2 rounded-xl text-[9px] font-black uppercase tracking-widest hover:bg-blue-700 shadow-lg transition-all animate-in fade-in slide-in-from-right-4"
                                    >
                                        <PrintIcon className="w-3.5 h-3.5 mr-2" /> Generate Statement (${selectedInvoiceIds.size})
                                    </button>
                                )}
                            </div>
                                    ...
                                    <table className="w-full text-sm text-left">
                                            ...
                                            <tr>
                                                <th className="px-5 py-3 w-10">
                                                    <input
                                                        ...
                                                        checked={selectedInvoiceIds.size === customerSales.length && customerSales.length > 0}
                                                        className="rounded border-gray-300 text-blue-600 focus:ring-blue-500"
                                                    />
                                                </th>
                                                <th className="px-5 py-3">REF / PAYMENTS</th>
                                        ...
                                        <tbody className="divide-y divide-gray-100">
                                            {customerSales.map(sale => {
                                                ...
                                                return (
                                                    <tr key={sale.id} className="bg-white hover:bg-gray-50 border-b border-gray-50 last:border-0">
                                                        <td className="px-5 py-4 align-top">
                                                            <input
                                                                type="checkbox"
                                                                checked={selectedInvoiceIds.has(sale.id)}
                                                                onChange={() => toggleInvoiceSelection(sale.id)}
                                                                className="rounded border-gray-300 text-blue-600 focus:ring-blue-500"
                                                            />
                                                        </td>
                                                        <td className="px-5 py-4 align-top">
                                                            <div className="font-mono font-bold text-blue-600 text-xs">#{sale.id.substring(0, 8).toUpperCase()}</div>
                                                            {sale.payments && sale.payments.length > 0 && (
                                                                <div className="mt-3 space-y-1.5">
                                                                    <p className="text-[8px] font-black text-gray-400 uppercase tracking-widest border-b border-gray-100 pb-1">Payment History</p>
                                                                    {sale.payments.map(p => (
                                                                        <div key={p.id} className="flex justify-between items-center text-[9px] font-bold text-emerald-600 bg-emerald-50/50 px-2 py-1 rounded-lg">
                                                                            <span>{new Date(p.date).toLocaleDateString([], { month: 'short', day: 'numeric' })}</span>
                                                                            <span>{formatUGX(p.amount).replace(' UGX', '')}</span>
                                                                        </div>
                                                                    ))}
                                                                </div>
                                                            )}
                                                        </td>
                                                        <td className="px-5 py-4 text-[10px] text-gray-500 font-bold align-top">{new Date(sale.date).toLocaleDateString()}</td>
                                                        <td className="px-5 py-4 text-right align-top">
                                                            <div className="font-black text-gray-900 text-xs">{formatUGX(sale.total)}</div>
                                                            {balance > 0 && <div className="text-[9px] font-bold text-red-500 mt-1">Bal: {formatUGX(balance)}</div>}
                                                        </td>
                                                        <td className="px-5 py-4 text-right align-top">
                                                        ...
                                                        <td className="px-5 py-4 text-center align-top">
                                                            <button onClick={() => setInvoiceToView(sale)} className="text-blue-600 hover:text-blue-800 transition-colors p-2 hover:bg-blue-50 rounded-xl shadow-sm"><DocumentTextIcon className="w-4 h-4" /></button>
                                                        </td>
                                                    </tr>
                                                );
                                            })}
